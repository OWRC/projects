...
read.csv("E:/Sync/@dev/go/src/ORMGP Modelling/HEC-HMS/event-summary-MadRiver23.csv") %>%
  ...
  arrange(year) %>%
  kable(caption = 'Extreme event summary', digits=1, format = "html", table.attr = "style='width:80%;'") %>%
...
SOLRIS land use mapping is also used to distribute vegetation canopy cover that provides first estimates of initial abstraction capacities:
...
Surficial Geology (OGS, 2010) defines soil characteristics. The OGS layers are attributed with a set of "relative permeabilities" needed to define ["Hydrologic Soil Groups"](https://directives.sc.egov.usda.gov/OpenNonWebContent.aspx?content=17757.wba). Based on the *"PERMEABILI"* attribute of the OGS (2010) layer, soil characteristics needed to estimate infiltration loss parameters for the Upper Mad River Watershed are determined:
...
*SCS Curve Numbers based on a geospatial overlay of SOLRIS and OGS surficial geology.*
...
[Subbasin parameters can be inspected above.](#sws.leaflet)
