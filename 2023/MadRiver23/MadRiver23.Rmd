...
df <- read.csv("M:/MadRiver23/modelling/02-goHECHMS/HEC-HMS_subbasins.csv")
```
...
subbasins <- st_transform(st_read("M:/MadRiver23/modelling/HEC-HMS/MadRiver23/maps/HEC-HMS_subbasins.shp", quiet=TRUE), 4326)
...
```{r results-load, include=FALSE}
  df.results.timmins <- read_excel("M:/MadRiver23/modelling/HEC-HMS/results/scenarios.xlsx",'Timmins')
  df.results.scsii <- read_excel("M:/MadRiver23/modelling/HEC-HMS/results/scenarios.xlsx",'SCSII')
  df.results.CC <- read_excel("M:/MadRiver23/modelling/HEC-HMS/results/scenarios.xlsx",'climate change')
```
...
1. 3 climate change scenarios (SSP1-2.4, SSP2-4.5 and SSP5-8.5) are considered.
...
df.results.CC %>%
  ...
  geom_ribbon(aes(ymin=`CC-2015-8.5-2yr`,ymax=`CC-2015-8.5-100yr`,fill='SSP5-8.5'),alpha=.5) +
  
  geom_ribbon(aes(ymin=`CC-2015-4.5-2yr`,ymax=`CC-2015-4.5-100yr`,fill='SSP2-4.5'),alpha=.5) +
  geom_ribbon(aes(ymin=`CC-2015-2.6-2yr`,ymax=`CC-2015-2.6-100yr`,fill='SSP1-2.6'),alpha=.5) +
...
df.results.CC %>%
  ...
  geom_ribbon(aes(ymin=`CC-2045-8.5-2yr`,ymax=`CC-2045-8.5-100yr`,fill='SSP5-8.5'),alpha=.5) +
  
  geom_ribbon(aes(ymin=`CC-2045-4.5-2yr`,ymax=`CC-2045-4.5-100yr`,fill='SSP2-4.5'),alpha=.5) +
  geom_ribbon(aes(ymin=`CC-2045-2.6-2yr`,ymax=`CC-2045-2.6-100yr`,fill='SSP1-2.6'),alpha=.5) +
